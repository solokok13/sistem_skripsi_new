    ...
    if file:
        ...
        try:
            ...
            for index, row in results.iterrows():
                app.logger.info(f"Inserting row: {row.to_dict()}")
                cur.execute("""
                    INSERT INTO predictions (nisn, nama, sikap, peng, ket, pts, pas, prediction, user_id, prediction_time)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (
                    row['NISN'], row['NAMA'], row['SIKAP'], row['peng'], row['ket'], row['PTS'], row['PAS'], row['prediction'], current_user.id, prediction_time
                ))
            mysql.connection.commit()
            cur.close()
...
def history():
    ...
    query = """
        SELECT nisn, nama, sikap, peng, ket, pts, pas, prediction, prediction_time 
